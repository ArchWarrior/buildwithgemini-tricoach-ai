    ...
    if os.path.exists(metadata_path):
        try:
            with open(metadata_path, "r") as f:
                ...
                RESOURCE = metadata.get("remote_agent_runtime_id")
        except Exception:
            pass

if not RESOURCE:
    raise RuntimeError("AGENT_ENGINE_RESOURCE_NAME environment variable or deployment_metadata.json is required.")
...
_TAG_RE = re.compile(r"<(a2ui-json|a2a_datapart_json)>([\s\S]*?)</\1>", re.IGNORECASE)
...
def _extract_parts(parts: list) -> list[dict]:
    ...
    application/json+a2ui) or inline <a2ui-json> blocks become {"kind": "a2ui", "data": <message>}
                    ...
                    try:
                        ...
                        if isinstance(parsed, dict):
                            if "data" in parsed and isinstance(parsed["data"], dict):
                                parsed = parsed["data"]
                            if "components" in parsed and not any(k in parsed for k in ("surfaceUpdate", "beginRendering")):
