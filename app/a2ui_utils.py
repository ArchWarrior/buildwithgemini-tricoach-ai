...
def _extract_a2ui_messages(text: str) -> list[dict]:
    ...
    for value in values:
        ...
        if isinstance(inner, dict):
            if any(k in inner for k in ("beginRendering", "surfaceUpdate", "dataModelUpdate", "deleteSurface")):
                messages.append(inner)
            elif "components" in inner and ("surfaceId" in inner or "root" in inner):
                messages.append({"surfaceUpdate": inner})
        elif any(k in value for k in ("beginRendering", "surfaceUpdate", "dataModelUpdate", "deleteSurface")):
            messages.append(value)
        elif "components" in value and ("surfaceId" in value or "root" in value):
